        ...
        basketItem.innerHTML = `
            ...
            <div class="basket-item-info">
               <img class="product-image" src="${item.image}">
                <div class="toggleBtn">
              <h3 class="basket-item-category">${item.category}</h3>
              <i class="fa-regular fa-circle-dot" style="color: #59e228;"></i></div>
              <div class="quantityBtns" >
            <button class="quantity-btn" onclick="changeQuantity(${item.id}, -1)">-</button>
                <span class="basket-item-quantity">${item.quantity}</span>
                <button class="quantity-btn" onclick="changeQuantity(${item.id}, 1)">+</button></div>
          <div class="totalPriceforEachProduct">
              <b>  <span class="basket-item-price">$${(item.price * item.quantity).toFixed(2)}</span></b>
               <a href="#" class="" onclick="">Save for later</a>
                <a href="#" class="remove-item" onclick="removeFromBasket(${item.id})">Remove</a></div>
            </div>
        `;
...
function removeFromBasket(productId) {
    ...
    if (product) {
        alert(`Removing ${product.title} from the basket.`);
        basket = basket.filter(item => item.id !== productId);
        saveBasket();
        updateBasketUI();
    }
...
function removeAllFromBasket(){
    alert('Removing all items from the basket.');
    basket = [];
    saveBasket();
    updateBasketUI(); 
}
